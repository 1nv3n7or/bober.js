...
module.exports.parse = function (filename) {
  ...
  const { body } = espree.parse(file, { ecmaVersion });
    ...
    switch (body[key].type) {
      case "VariableDeclaration":
        if (body[key]?.declarations?.[0]?.init?.callee?.name === "require") {
          ...
          body[key]?.declarations?.[0]?.init?.arguments?.[0]?.callee?.name ===
          "require"
        ) {
          requires.push({
            module:
              body[key].declarations[0].init.arguments[0].arguments[0].value,
          });
        } else if (
          body[key].declarations?.[0]?.init?.object?.callee?.name === "require"
        ) {
          requires.push({
      ...
      case "FunctionDeclaration":
        if (body[key]?.type === "FunctionDeclaration") {
          ...
          body[key]?.expression?.right?.type === "FunctionExpression"
        ) {
